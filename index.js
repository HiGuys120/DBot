...
bot.on('message',(message)=>{
	if(message.content.charAt(0)=='!'){
		...
		if(command.startsWith('getAnime')){
			...
			request('http://www.omdbapi.com/?t='+movieName+'&apikey='+omdbpApiKey,{json:true},(error,response,body)=>{
			...
			request('https://newsapi.org/v2/top-headlines?sources='+newsSource+'&apiKey='+newsOrgApiKey,{json:true},(error,response,body)=>{
				if(body.status == 'ok'){
			...
			request('http://api.wunderground.com/api/'+wundergroundApiKey+'/conditions/q/ES/'+place+'.json',{json:true},(error,response,body)=>{
